...
from run_utils import (
    add_new_clasifications_to_players_teams,
    get_ball_detections,
    get_main_ball,
    get_player_detections,
    set_detections_classification,
    should_classify,
...
player_path_drawer = AbsolutePaths(max_history=40, thickness=2)
...
for frame in video:
    ...
    # Get Detections
    players_detections = get_player_detections(player_detector, hsv_classifier, frame)
    ...
    detections_for_classification = [
        detection for detection in player_detections if should_classify(detection)
    ]

    detections_classified = hsv_classifier.predict_from_detections(
        detections=detections_for_classification, img=frame
    )

    add_new_clasifications_to_players_teams(detections_classified)

    set_detections_classification(player_detections)
